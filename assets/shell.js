document.addEventListener('DOMContentLoaded', () => {
  ...
  const injectScopedStylesheet = async (link, scopeTarget) => {
    ...
    const scopeSelector = '.legacy-page-shell';
    ...
    const effectiveScope = targetScopeClass || scopeSelector;

    link.remove();

    try {
      const scopedCssText = await fetchScopedStylesheet(absoluteUrl.href, effectiveScope);
